...
function ActionBox:init(x, y, index, battler)
    ...
    self.status_offset_x, self.head_offset_y = battler.chara:getStatusIconOffset()

    self.head_sprite = Sprite(battler.chara:getHeadIcons().."/"..battler:getHeadIcon(), 18 + self.head_offset_x, 11 + self.head_offset_y)
    ...
    self.status_sprite = Sprite(battler.chara:getStatusIcons().."/"..battler:getStatusIcon(), 62 + self.status_offset_x, 11 + self.head_offset_y)
    if not self.status_sprite:getTexture() then
        self.status_sprite:setSprite(battler.chara:getStatusIcons().."/none")
    end
    ...
    self.hp_sprite = Sprite("ui/hp", 115, 10)
    self.ad_sprite = Sprite("ui/ad", 115, -16)

    self.box:addChild(self.head_sprite)
    self.box:addChild(self.hp_sprite)
    self.box:addChild(self.ad_sprite)
    self.box:addChild(self.status_sprite)
...
function ActionBox:createButtons()
    ...
    for i,btn in ipairs(btn_types) do
        if type(btn) == "string" then
            ...
            local button = ActionButton(btn, self.battler, 80 + 20 - self.x + h*48, -420 + (j)*31)
            button.posx = 80 + 20 - self.x + h*48
            button.offset = 0
            button.actbox = self
    ...
    self.selected_button = Utils.clamp(self.selected_button, 1 - 4, #self.buttons + 4)
end
...
function ActionBox:setStatusIcon(icon)
    self.force_head_sprite = true

    local full_icon = self.battler.chara:getStatusIcons().."/"..icon
    if self.status_sprite:hasSprite(full_icon) then
        self.status_sprite:setSprite(full_icon)
    else
        self.status_sprite:setSprite(self.battler.chara:getStatusIcons().."/none")
    end
end
...
function ActionBox:resetStatusIcon()
    self.force_head_sprite = false

    local full_icon = self.battler.chara:getStatusIcons().."/"..self.battler:getStatusIcon()
    if self.status_sprite:hasSprite(full_icon) then
        self.status_sprite:setSprite(full_icon)
    else
        self.status_sprite:setSprite(self.battler.chara:getStatusIcons().."/none")
...
function ActionBox:update()
    ...
    if Game.battle.current_selecting == self.index then
        if self.box.y > -8  then self.box.y = self.box.y - 4 * DTMULT end
    ...
    self.status_sprite.y = 8 - self.data_offset
    if self.name_sprite then
        self.name_sprite.y = 14 - self.data_offset
    end
    self.hp_sprite.y = 10 - self.data_offset
    self.ad_sprite.y = -16 - self.data_offset
    ...
    if not self.force_head_sprite then
        local current_status = self.battler.chara:getStatusIcons().."/"..self.battler:getStatusIcon()
        if not self.status_sprite:hasSprite(current_status) then
            current_status = self.battler.chara:getStatusIcons().."/none"
        end

        if not self.status_sprite:isSprite(current_status) then
            self.status_sprite:setSprite(current_status)
        end
    end
