...
function DarkEquipMenu:init()
    ...
    self.caption_sprites = {
        ...
        ["tomes"] = Assets.getTexture("ui/menu/caption_tomes")
    }
...
function DarkEquipMenu:getCurrentItemType()
    if self.selected_slot == 1 then
        return "weapons"
    elseif self.selected_slot < 4 then
        return "armors"
    else
        return "tomes"
    end
...
function DarkEquipMenu:canEquipSelected()
    ...
    elseif self.selected_slot < 4 then
        ...
        return character:canEquip(item, "tome", self.selected_slot)
    end
end
...
function DarkEquipMenu:getEquipPreview()
    ...
    if self.selected_slot == 4 then
        equipped[4] = item
    else
        equipped[4] = party.equipped.tome
    end
    return equipped
end
...
function DarkEquipMenu:update()
    ...
    elseif self.state == "SLOTS" then
        ...
        self.selected_slot = (self.selected_slot - 1) % 4 + 1
        ...
        if Input.pressed("confirm") then
            ...
            if not self:canEquipSelected() then
                ...
                local swap_with = (self.selected_slot == 1) and party:getWeapon() or
                    party:getArmor(self.selected_slot - 1) or party:getTome()
...
function DarkEquipMenu:draw()
    ...
    Draw.setColor(PALETTE["world_border"]) -- add 8
    love.graphics.rectangle("fill", 188, -24, 6, 139 + 8)
    love.graphics.rectangle("fill", -24, 109 + 8, 58, 6)
    love.graphics.rectangle("fill", 130, 109 + 8, 160, 6)
    love.graphics.rectangle("fill", 422, 109 + 8, 81, 6)
    love.graphics.rectangle("fill", 241, 109 + 8, 6, 192 - 8)

    Draw.setColor(1, 1, 1, 1)
    Draw.draw(self.caption_sprites["char"], 36, -26, 0, 2, 2)
    Draw.draw(self.caption_sprites["equipped"], 294, -26, 0, 2, 2)
    Draw.draw(self.caption_sprites["stats"], 34, 104 + 7, 0, 2, 2) -- add 7
    if self.selected_slot == 1 then -- add 7
        Draw.draw(self.caption_sprites["weapons"], 290, 104 + 7, 0, 2, 2)
    elseif self.selected_slot < 4 then
        Draw.draw(self.caption_sprites["armors"], 290, 104 + 7, 0, 2, 2)
    else
        Draw.draw(self.caption_sprites["tomes"], 290, 104 + 7, 0, 2, 2)
    end
...
function DarkEquipMenu:drawEquipped()
    ...
    if self.state ~= "SLOTS" or self.selected_slot ~= 1 then
        ...
        if weapon_icon then
            Draw.draw(weapon_icon, 220, -14 - 2, 0, 2, 2)
        end
    end -- subtract 2
    if self.state ~= "SLOTS" or self.selected_slot ~= 2 then Draw.draw(self.armor_icons[1], 220, 20 - 2, 0, 2, 2) end
    if self.state ~= "SLOTS" or self.selected_slot ~= 3 then Draw.draw(self.armor_icons[2], 220, 50 - 2, 0, 2, 2) end
    if self.state ~= "SLOTS" or self.selected_slot ~= 4 then
       local tome_icon = Assets.getTexture(party:getTomeIcon())
       --local tome_icon = Assets.getTexture("ui/menu/equip/tome")
        if tome_icon then
            Draw.draw(tome_icon, 220, 82 - 2, 0, 2, 2) 
        end
    end

    if self.state == "SLOTS" then
        Draw.setColor(Game:getSoulColor())
        Draw.draw(self.heart_sprite, 226, 10 + ((self.selected_slot - 1) * 30) - 10 - 2)
    end

    for i = 1, 4 do
        self:drawEquippedItem(i, 261, 6 + ((i - 1) * 30) - 10 - 2)
    end
end
    ...
    for i = scroll, math.min(items.max, scroll + 5) do
        ...
        if item then
            ...
            elseif self.selected_slot < 4 then
                ...
                usable = party:canEquip(item, "tome", self.selected_slot)
            end
            if usable then
                Draw.setColor(1, 1, 1)
            else
            ...
            if item.icon and Assets.getTexture(item.icon) then --add 7
                Draw.draw(Assets.getTexture(item.icon), x, y + (offset * 27) + 7, 0, 2, 2)
            end
            love.graphics.print(item:getName(), x + 20, y + (offset * 27) + 7 - 6)
        else
            Draw.setColor(0.25, 0.25, 0.25)
            love.graphics.print("---------", x + 20, y + (offset * 27) + 7 - 6)
        end
    end

    if self.state == "ITEMS" then
        Draw.setColor(Game:getSoulColor())
        Draw.draw(self.heart_sprite, x - 20, y + 4 + ((self.selected_item[type] - scroll) * 27) + 6)
...
function DarkEquipMenu:drawStats()
    local party = self.party:getSelected()
    Draw.setColor(1, 1, 1, 1)
    Draw.draw(self.stat_icons["attack"], -8, 134 - 3, 0, 2, 2)
    Draw.draw(self.stat_icons["defense"], -8, 161 - 3, 0, 2, 2)
    Draw.draw(self.stat_icons["magic"], -8, 188 - 3, 0, 2, 2)
    love.graphics.print("Attack:", 18, 128 - 3)
    love.graphics.print("Defense:", 18, 155 - 3)
    love.graphics.print("Magic:", 18, 182 - 3)
    local stats, compare = self:getStatsPreview()
    self:drawStatPreview("attack", 148, 128 - 3, stats, compare, true)
    self:drawStatPreview("defense", 148, 155 - 3, stats, compare, false)
    self:drawStatPreview("magic", 148, 182 - 3, stats, compare, false)
    local abilities, ability_comp = self:getAbilityPreview()
    for i = 1, 3 do
        self:drawAbilityPreview(i, -8, 188 - 3 + (27 * i), abilities, ability_comp)
    end
end
