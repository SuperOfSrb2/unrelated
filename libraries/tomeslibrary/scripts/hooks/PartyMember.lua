...
function PartyMember:init()
    ...
    self.tome_icon = "ui/menu/equip/tome"
...
function PartyMember:getTomeIcon() return self.tome_icon end
...
function PartyMember:getEquipment()
    ...
    if self.equipped.tome then
        table.insert(result, self.equipped.tome)
    end
    return result
end
...
function PartyMember:getTome()
    return self.equipped.tome
end
...
function PartyMember:setTome(item)
    if type(item) == "string" then
        item = Registry.createItem(item)
    end
    self.equipped.tome = item
end
...
function PartyMember:checkTome(id)
    return self:getTome() and self:getTome().id == id or false
end
...
function PartyMember:saveEquipment()
    ...
    if self.equipped.tome then
        result.tome = self.equipped.tome:save()
    end
    return result
end
...
function PartyMember:loadEquipment(data)
    ...
    if type(data.tome) == "table" then
        local tome = Registry.createItem(data.tome.id)
        tome:load(data.tome)
        self:setTome(tome)
    else
        self:setTome(data.tome)
    end
end
